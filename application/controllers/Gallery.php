...
class Gallery extends MY_Controller {
	...
	public function view_full($id)
	{
		...
		/*$config['base_url'] = site_url('gallery/view_full/' . $id);
		...
		$config = pagination_config(array(
			'base_url' => site_url('gallery/view_full/' . $id),
			'total_rows' => $data['objects']['total_rows'],
			'per_page' => 1,
			'prev_link' => lang('app_back'),
			'next_link' => lang('app_forward'),
			'display_pages' => FALSE,
			'prev_tag_open' => '<div class="btn btn-info view_full_button">',
			'prev_tag_close' => '</div>',
			'next_tag_open' => '<div class="btn btn-info view_full_button">',
			'next_tag_close' => '</div>',
			'first_link' => FALSE,
			'last_link' => FALSE,
			));
